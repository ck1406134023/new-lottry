<template>
  <div class="home">
    ...
    <div class="tab-btns">
      <div
        v-for="(coin, index) in supportCoin"
        :key="index"
        class="btn"
        @click="switchToken(index)"
        :class="{active: index === activeToken}"
      >
        <img :src="coin.src">
    ...
    <div class="main">
      ...
      <div style="display:none">
        ...
        <div class="mobile-show componet-tab">
          <span
            :class="{'active-componet': currentComponent === 'dice-roll'}"
            @click="currentComponent = 'dice-roll'"
          >DICE</span>
          <span
            :class="{'active-componet': currentComponent === 'dice-auction'}"
            @click="currentComponent = 'dice-auction'"
          >RICH AUCTION</span>
        </div>
...
export default {
  ...
  computed: {
    ...
    token: {
      get() {
        return this.activeToken === "eos" ? "rich" : this.activeToken;
      },
      set(newValue) {
    ...
    token(newValue) {
      if (this.username) {
        this.$store.dispatch("UPDATE_TOKEN_ASYNC", {
          type: newValue === "eos" ? "rich" : newValue
        });
      }
    },
    username(newValue) {
      if (newValue) {
        this.$store.dispatch("UPDATE_TOKEN_ASYNC", {
          type: newValue === "eos" ? "rich" : this.token
        });
